...
import {
  ...
  CONSTRUCTION_MATERIALS_SPEC_CONFIG,
...
import { ConstructionMaterialsFilterState, EMPTY_CONSTRUCTION_MATERIALS_FILTERS } from './FilterSidebar';

// ─── Props ───────────────────────────────────────────────────────────────────
interface ConstructionMaterialsFilterEnhancedProps {
  locale: Locale;
  filters: ConstructionMaterialsFilterState;
  onFiltersChange: (filters: ConstructionMaterialsFilterState) => void;
  onClear?: () => void;
  onSearch?: () => void;
  resultCount?: number;
}
...
// ─── Main Component ──────────────────────────────────────────────────────────
export const ConstructionMaterialsFilterEnhanced: React.FC<ConstructionMaterialsFilterEnhancedProps> = ({
  locale,
  filters,
  onFiltersChange,
  onClear,
  ...
  const tCM = useTranslations('postAd');
  ...
  const setFilter = (updates: Partial<ConstructionMaterialsFilterState>) => {
    onFiltersChange({ ...filters, ...updates });
  };

  // ─── Translation helpers ─────────────────────────────────────────────────
  const getCMFieldLabel = (key: string, fallback: string): string => {
    try {
      const translated = tCM(`constructionMaterials.fields.${key}` as Parameters<typeof tCM>[0]);
      if (translated && !translated.includes('constructionMaterials.fields.')) {
        return translated;
      }
    } catch {
      // fall through
  ...
  const getCMOptionLabel = (key: string): string => {
    try {
      const translated = tCM(`constructionMaterials.optionLabels.${key}` as Parameters<typeof tCM>[0]);
      if (translated && !translated.includes('constructionMaterials.optionLabels.')) {
        return translated;
      }
    } catch {
      // fall through
  ...
  const getCMSubcategoryLabel = (key: string): string => {
    try {
      const labelKey = CONSTRUCTION_MATERIALS_SUBCATEGORY_LABEL_KEYS[key as ConstructionMaterialsSubcategory];
      if (labelKey) {
        const translated = tCM(`constructionMaterials.subcategories.${labelKey}` as Parameters<typeof tCM>[0]);
        if (translated && !translated.includes('constructionMaterials.subcategories.')) {
          return translated;
        }
      }
    } catch {
  ...
  const subcategoryOptions = useMemo(
    () =>
      CONSTRUCTION_MATERIALS_SUBCATEGORIES.map((sub) => ({
        value: sub.value,
        label: getCMSubcategoryLabel(sub.value),
      })),
  ...
  const quantityUnitOptions = useMemo(
    () =>
      CM_QUANTITY_UNIT_OPTIONS.map((unit) => ({
        value: unit,
        label: getCMOptionLabel(unit),
      })),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [locale],
  );
  ...
  const conditionOptions = useMemo(
    () =>
      CM_CONDITION_OPTIONS.map((opt) => ({
        value: opt,
        label: getCMOptionLabel(opt),
      })),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [locale],
  );
  ...
  const CM_COMMON_FIELD_KEYS = new Set([
    'quantity', 'unit', 'condition', 'price', 'priceType', 'deliveryAvailable', 'minOrder',
  ]);

  const specFields = useMemo(
    () =>
      filters.subcategory
        ? (CONSTRUCTION_MATERIALS_SPEC_CONFIG[filters.subcategory] || []).filter(
            (field) => !CM_COMMON_FIELD_KEYS.has(field.key),
          )
        : [],
    [filters.subcategory],
  );
  ...
  const renderSpecField = (field: ConstructionMaterialsSpecField) => {
    const key = field.key as keyof ConstructionMaterialsFilterState;
    const label = getCMFieldLabel(field.key, field.label);
    ...
    if (field.type === 'toggle') {
      ...
      return (
        <ToggleField
          ...
          label={label}
          value={value}
          onChange={(v) => setFilter({ [key]: v } as Partial<ConstructionMaterialsFilterState>)}
          isRtl={rtl}
    ...
    if (field.type === 'unit') {
      const unitKey = `${key}Unit` as keyof ConstructionMaterialsFilterState;
      const value = String(filters[key] || '');
      const unitValue = String(filters[unitKey] || '');
      const unitOpts = field.unitOptions
        ? field.unitOptions.map((u) => ({ value: u, label: getCMOptionLabel(u) }))
        : quantityUnitOptions;
      return (
        <UnitField
          key={field.key}
          ...
          value={value}
          unitValue={unitValue}
          onValueChange={(v) => setFilter({ [key]: v } as Partial<ConstructionMaterialsFilterState>)}
          onUnitChange={(v) => setFilter({ [unitKey]: v } as Partial<ConstructionMaterialsFilterState>)}
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SelectField
            ...
            onChange={(v) => setFilter({ ...EMPTY_CONSTRUCTION_MATERIALS_FILTERS, subcategory: v as ConstructionMaterialsSubcategory | '' })}
      ...
      <FilterSection
        title={getCMFieldLabel('productDetails', 'Product Details')}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <NumberField
            label={getCMFieldLabel('quantity', 'Quantity')}
            value={filters.quantity}
            onChange={(v) => setFilter({ quantity: v })}
            isRtl={rtl}
            placeholder={getCMFieldLabel('quantity', 'Quantity')}
            min="0"
          />
          <SelectField
            label={getCMFieldLabel('unit', 'Unit')}
            value={filters.unit}
          ...
          />
          <SelectField
            label={getCMFieldLabel('condition', 'Condition')}
          ...
          />
          <SelectField
            label={getCMFieldLabel('deliveryAvailable', 'Delivery')}
          ...
          />
          <TextField
            label={getCMFieldLabel('minOrder', 'Minimum Order')}
            value={filters.minOrder}
            onChange={(v) => setFilter({ minOrder: v })}
            isRtl={rtl}
            placeholder={getCMFieldLabel('minOrder', 'Minimum Order')}
          />
        </div>
      </FilterSection>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <NumberField
            label={getCMFieldLabel('price', 'Price')}
            value={filters.price}
            onChange={(v) => setFilter({ price: v })}
            isRtl={rtl}
            placeholder={getCMFieldLabel('price', 'Price')}
            min="0"
          />
          <SelectField
            label={getCMFieldLabel('priceType', 'Price Type')}
      ...
      <div className={`flex gap-2 ${rtl ? 'flex-row-reverse' : ''}`}>
        <button
          type="button"
          onClick={() => {
            setFilter(EMPTY_CONSTRUCTION_MATERIALS_FILTERS);
            onClear?.();
          }}
...
export default ConstructionMaterialsFilterEnhanced;
