...
import {
  ...
  FOOD_AGRICULTURE_SUBCATEGORIES,
  FOOD_AGRICULTURE_SPEC_CONFIG,
...
import { FoodAgricultureFilterState, EMPTY_FOOD_AGRICULTURE_FILTERS } from './FilterSidebar';

// ─── Props ───────────────────────────────────────────────────────────────────
interface FoodAgricultureFilterEnhancedProps {
  locale: Locale;
  filters: FoodAgricultureFilterState;
  onFiltersChange: (filters: FoodAgricultureFilterState) => void;
  onClear?: () => void;
  onSearch?: () => void;
  resultCount?: number;
}
...
// ─── Main Component ──────────────────────────────────────────────────────────
export const FoodAgricultureFilterEnhanced: React.FC<FoodAgricultureFilterEnhancedProps> = ({
  locale,
  filters,
  onFiltersChange,
  onClear,
  ...
  const tFA = useTranslations('postAd');
  ...
  const setFilter = (updates: Partial<FoodAgricultureFilterState>) => {
    onFiltersChange({ ...filters, ...updates });
  };

  // ─── Translation helpers ─────────────────────────────────────────────────
  const getFAFieldLabel = (key: string, fallback: string): string => {
    try {
      const translated = tFA(`foodAgriculture.fields.${key}` as Parameters<typeof tFA>[0]);
      if (translated && !translated.includes('foodAgriculture.fields.')) {
        return translated;
      }
    } catch {
      // fall through
  ...
  const getFAOptionLabel = (key: string): string => {
    try {
      const translated = tFA(`foodAgriculture.optionLabels.${key}` as Parameters<typeof tFA>[0]);
      if (translated && !translated.includes('foodAgriculture.optionLabels.')) {
        return translated;
      }
    } catch {
      // fall through
  ...
  const getFASubcategoryLabel = (key: string): string => {
    try {
      const translated = tFA(`foodAgriculture.subcategories.${key}` as Parameters<typeof tFA>[0]);
      if (translated && !translated.includes('foodAgriculture.subcategories.')) {
        return translated;
      }
    } catch {
      // fall through
  ...
  const subcategoryOptions = useMemo(
    () =>
      FOOD_AGRICULTURE_SUBCATEGORIES.map((sub) => ({
        value: sub.value,
        label: getFASubcategoryLabel(sub.value),
      })),
  ...
  const FOOD_AGRICULTURE_COMMON_FIELD_KEYS = new Set([
    'quantity', 'unit', 'grade', 'freshness', 'origin', 'certification', 'price', 'priceType', 'deliveryAvailable', 'minOrder',
  ]);

  const specFields = useMemo(
    () =>
      filters.subcategory
        ? (FOOD_AGRICULTURE_SPEC_CONFIG[filters.subcategory] || []).filter(
            (field) => !FOOD_AGRICULTURE_COMMON_FIELD_KEYS.has(field.key),
          )
        : [],
    [filters.subcategory],
  );
  ...
  const renderSpecField = (field: FoodAgricultureSpecField) => {
    const key = field.key as keyof FoodAgricultureFilterState;
    const label = getFAFieldLabel(field.key, field.label);
    ...
    if (field.type === 'toggle') {
      ...
      return (
        <ToggleField
          ...
          label={label}
          value={value}
          onChange={(v) => setFilter({ [key]: v } as Partial<FoodAgricultureFilterState>)}
          isRtl={rtl}
    ...
    if (field.type === 'unit') {
      const unitKey = `${key}Unit` as keyof FoodAgricultureFilterState;
      const value = String(filters[key] || '');
      const unitValue = String(filters[unitKey] || '');
      const unitOpts = field.unitOptions
        ? field.unitOptions.map((u) => ({ value: u, label: getFAOptionLabel(u) }))
        : quantityUnitOptions;
      return (
        <UnitField
          key={field.key}
          ...
          value={value}
          unitValue={unitValue}
          onValueChange={(v) => setFilter({ [key]: v } as Partial<FoodAgricultureFilterState>)}
          onUnitChange={(v) => setFilter({ [unitKey]: v } as Partial<FoodAgricultureFilterState>)}
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SelectField
            ...
            onChange={(v) => setFilter({ ...EMPTY_FOOD_AGRICULTURE_FILTERS, subcategory: v as FoodAgricultureSubcategory | '' })}
      ...
      <FilterSection
        title={getFAFieldLabel('productDetails', 'Product Details')}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <NumberField
            label={getFAFieldLabel('quantity', 'Quantity')}
            value={filters.quantity}
            onChange={(v) => setFilter({ quantity: v })}
            isRtl={rtl}
            placeholder={getFAFieldLabel('quantity', 'Quantity')}
            min="0"
          />
          <SelectField
            label={getFAFieldLabel('unit', 'Unit')}
            value={filters.unit}
          ...
          />
          <SelectField
            label={getFAFieldLabel('grade', 'Grade/Quality')}
            value={filters.grade}
          ...
          />
          <SelectField
            label={getFAFieldLabel('freshness', 'Freshness/Condition')}
          ...
          />
          <TextField
            label={getFAFieldLabel('origin', 'Origin/Source')}
            value={filters.origin}
            onChange={(v) => setFilter({ origin: v })}
            isRtl={rtl}
            placeholder={getFAFieldLabel('origin', 'Origin/Source')}
          />
          <SelectField
            label={getFAFieldLabel('certification', 'Certification')}
          ...
          />
          <SelectField
            label={getFAFieldLabel('deliveryAvailable', 'Delivery')}
          ...
          />
          <TextField
            label={getFAFieldLabel('minOrder', 'Minimum Order')}
            value={filters.minOrder}
            onChange={(v) => setFilter({ minOrder: v })}
            isRtl={rtl}
            placeholder={getFAFieldLabel('minOrder', 'Minimum Order')}
          />
        </div>
      </FilterSection>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <NumberField
            label={getFAFieldLabel('price', 'Price')}
            value={filters.price}
            onChange={(v) => setFilter({ price: v })}
            isRtl={rtl}
            placeholder={getFAFieldLabel('price', 'Price')}
            min="0"
          />
          <SelectField
            label={getFAFieldLabel('priceType', 'Price Type')}
      ...
      <div className={`flex gap-2 ${rtl ? 'flex-row-reverse' : ''}`}>
        <button
          type="button"
          onClick={() => {
            setFilter(EMPTY_FOOD_AGRICULTURE_FILTERS);
            onClear?.();
          }}
...
export default FoodAgricultureFilterEnhanced;
