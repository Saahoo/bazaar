...
import { BabyKidsFilterState, EMPTY_BABY_KIDS_FILTERS } from './FilterSidebar';

// ─── Props ───────────────────────────────────────────────────────────────────
interface BabyKidsFilterEnhancedProps {
  locale: Locale;
  filters: BabyKidsFilterState;
  onFiltersChange: (filters: BabyKidsFilterState) => void;
  onClear?: () => void;
  onSearch?: () => void;
  resultCount?: number;
}
...
// ─── Main Component ──────────────────────────────────────────────────────────
export const BabyKidsFilterEnhanced: React.FC<BabyKidsFilterEnhancedProps> = ({
  locale,
  filters,
  onFiltersChange,
  onClear,
  ...
  const tBK = useTranslations('postAd');
  const tCommon = useTranslations('common');
  const rtl = isRTL(locale);

  const [openSections, setOpenSections] = useState({
    general: true,
    babyKidsDetails: true,
    pricing: false,
  ...
  const setFilter = (updates: Partial<BabyKidsFilterState>) => {
    onFiltersChange({ ...filters, ...updates });
  };

  // ─── Translation helpers ─────────────────────────────────────────────────
  const getBKFieldLabel = (key: string, fallback: string): string => {
    try {
      const translated = tBK(`babyKids.fields.${key}` as Parameters<typeof tBK>[0]);
      if (translated && !translated.includes('babyKids.fields.')) {
        return translated;
      }
    } catch {
      // fall through
  ...
  const getBKOptionLabel = (key: string): string => {
    try {
      const translated = tBK(`babyKids.optionLabels.${key}` as Parameters<typeof tBK>[0]);
      if (translated && !translated.includes('babyKids.optionLabels.')) {
        return translated;
      }
    } catch {
      // fall through
  ...
  const getBKSubcategoryLabel = (key: string): string => {
    try {
      const translated = tBK(`babyKids.subcategories.${key}` as Parameters<typeof tBK>[0]);
      if (translated && !translated.includes('babyKids.subcategories.')) {
        return translated;
      }
    } catch {
      // fall through
  ...
  const subcategoryOptions = useMemo(
    () =>
      BABY_KIDS_SUBCATEGORIES.map((sub) => ({
        value: sub.value,
        label: getBKSubcategoryLabel(sub.value),
      })),
  ...
  const BABY_KIDS_COMMON_FIELD_KEYS = new Set([
    'condition', 'ageRange', 'gender', 'price', 'priceType', 'deliveryAvailable',
  ]);

  const specFields = useMemo(
    () =>
      filters.subcategory
        ? (BABY_KIDS_SPEC_CONFIG[filters.subcategory] || []).filter(
            (field) => !BABY_KIDS_COMMON_FIELD_KEYS.has(field.key),
          )
        : [],
    [filters.subcategory],
  );
  ...
  const renderSpecField = (field: BabyKidsSpecField) => {
    const key = field.key as keyof BabyKidsFilterState;
    const label = getBKFieldLabel(field.key, field.label);
    ...
    if (field.type === 'toggle') {
      ...
      return (
        <ToggleField
          ...
          label={label}
          value={value}
          onChange={(v) => setFilter({ [key]: v } as Partial<BabyKidsFilterState>)}
          isRtl={rtl}
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SelectField
            ...
            onChange={(v) => setFilter({ ...EMPTY_BABY_KIDS_FILTERS, subcategory: v as BabyKidsSubcategory | '' })}
      ...
      {/* ── Section 2: Baby & Kids Details ───────────────────────────────── */}
      <FilterSection
        title={getBKFieldLabel('babyKidsDetails', 'Baby & Kids Details')}
        icon={<BabyKidsDetailsIcon />}
        isOpen={openSections.babyKidsDetails}
        onToggle={() => toggleSection('babyKidsDetails')}
        isRtl={rtl}
        accentColor="pink"
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SelectField
            label={getBKFieldLabel('condition', 'Condition')}
          ...
          />
          <SelectField
            label={getBKFieldLabel('ageRange', 'Age Range')}
          ...
          />
          <SelectField
            label={getBKFieldLabel('gender', 'Gender')}
          ...
          />
          <SelectField
            label={getBKFieldLabel('deliveryAvailable', 'Delivery')}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <NumberField
            label={getBKFieldLabel('price', 'Price')}
            value={filters.price}
            onChange={(v) => setFilter({ price: v })}
            isRtl={rtl}
            placeholder={getBKFieldLabel('price', 'Price')}
            min="0"
          />
          <SelectField
            label={getBKFieldLabel('priceType', 'Price Type')}
      ...
      <div className={`flex gap-2 ${rtl ? 'flex-row-reverse' : ''}`}>
        <button
          type="button"
          onClick={() => {
            setFilter(EMPTY_BABY_KIDS_FILTERS);
            onClear?.();
          }}
...
export default BabyKidsFilterEnhanced;
