...
import { FoodAgricultureSubcategory, FOOD_AGRICULTURE_SUBCATEGORIES } from '@/lib/constants/food-agriculture-wizard';
import { getCategoryName } from '@/lib/constants/categories';
import { InputField } from './FoodAgricultureFieldControls';

interface StepFoodAgricultureBasicInfoProps {
  locale: Locale;
  data: {
    title: string;
    description: string;
  ...
  onChange: (updates: Partial<{ title: string; description: string; subcategory: FoodAgricultureSubcategory | '' }>) => void;
}

export const StepFoodAgricultureBasicInfo: React.FC<StepFoodAgricultureBasicInfoProps> = ({ locale, data, onChange }) => {
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => onChange({ subcategory: (e.target.value as FoodAgricultureSubcategory) || '' })}
            className={`w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm shadow-sm transition focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-100 ${rtl ? 'text-right' : 'text-left'}`}
            dir={rtl ? 'rtl' : 'ltr'}
          ...
          >
            ...
            {FOOD_AGRICULTURE_SUBCATEGORIES.map((item) => (
