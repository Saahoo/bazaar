...
import { FoodAgricultureSubcategory, FOOD_AGRICULTURE_SUBCATEGORIES, getFoodAgricultureFieldTranslationKey, getFoodAgricultureOptionTranslationKey } from '@/lib/constants/food-agriculture-wizard';
import { FoodAgricultureMediaData } from './StepFoodAgricultureMedia';
import { UploadedPhoto } from '@/components/post-ad/ImageUploader';

interface StepFoodAgricultureReviewProps {
  locale: Locale;
  title: string;
  description: string;
...
export const StepFoodAgricultureReview: React.FC<StepFoodAgricultureReviewProps> = ({
  locale,
  title,
  description,
  subcategory,
  ...
  const subcategoryLabel = subcategory
    ? FOOD_AGRICULTURE_SUBCATEGORIES.find((cat) => cat.value === subcategory)?.label || subcategory
    : '-';

  const getFieldLabel = (key: string) => {
    const translationKey = getFoodAgricultureFieldTranslationKey(key);
    return t.has(translationKey as Parameters<typeof t>[0]) ? t(translationKey as Parameters<typeof t>[0]) : key;
  };

  const getOptionLabel = (value: string) => {
    const translationKey = getFoodAgricultureOptionTranslationKey(value);
  ...
  const sections = [
    ...
    },
    {
      ...
      rows: [
        ...
        ['grade', stringifyValue(grade)],
        ['freshness', stringifyValue(freshness)],
        ['origin', origin || '-'],
        ['certification', stringifyValue(certification)],
