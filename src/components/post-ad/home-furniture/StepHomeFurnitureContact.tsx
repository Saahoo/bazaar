...
interface StepHomeFurnitureContactProps {
  locale: Locale;
...
export const StepHomeFurnitureContact: React.FC<StepHomeFurnitureContactProps> = ({
  locale,
  data,
  onChange,
}) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label
            className={`mb-1.5 block text-sm font-semibold text-slate-700 ${rtl ? 'text-right' : 'text-left'}`}
          >
      ...
      <div>
        ...
        <p className={`mb-3 text-xs text-slate-500 ${rtl ? 'text-right' : 'text-left'}`}>
          {t('mapHint')}
        </p>
        <LocationMap
          lat={data.lat}
      ...
      <label
        className={`inline-flex items-start gap-2 text-sm text-slate-700 ${rtl ? 'flex-row-reverse' : ''}`}
      >
        <input
          type="checkbox"
