...
import { ShoppingGroceriesSubcategory, SHOPPING_GROCERIES_SUBCATEGORIES } from '@/lib/constants/shopping-groceries-wizard';
import { getCategoryName } from '@/lib/constants/categories';
import { InputField } from './ShoppingGroceriesFieldControls';

interface StepShoppingGroceriesBasicInfoProps {
  locale: Locale;
  data: {
    title: string;
    description: string;
  ...
  onChange: (updates: Partial<{ title: string; description: string; subcategory: ShoppingGroceriesSubcategory | '' }>) => void;
}

export const StepShoppingGroceriesBasicInfo: React.FC<StepShoppingGroceriesBasicInfoProps> = ({ locale, data, onChange }) => {
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => onChange({ subcategory: (e.target.value as ShoppingGroceriesSubcategory) || '' })}
            className={`w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm shadow-sm transition focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-100 ${rtl ? 'text-right' : 'text-left'}`}
            dir={rtl ? 'rtl' : 'ltr'}
          ...
          >
            ...
            {SHOPPING_GROCERIES_SUBCATEGORIES.map((item) => (
