...
import { ShoppingGroceriesSubcategory, SHOPPING_GROCERIES_SUBCATEGORIES, getShoppingGroceriesFieldTranslationKey, getShoppingGroceriesOptionTranslationKey } from '@/lib/constants/shopping-groceries-wizard';
import { ShoppingGroceriesMediaData } from './StepShoppingGroceriesMedia';
import { UploadedPhoto } from '@/components/post-ad/ImageUploader';

interface StepShoppingGroceriesReviewProps {
  locale: Locale;
  title: string;
  description: string;
...
export const StepShoppingGroceriesReview: React.FC<StepShoppingGroceriesReviewProps> = ({
  locale,
  title,
  description,
  subcategory,
  ...
  const subcategoryLabel = subcategory
    ? SHOPPING_GROCERIES_SUBCATEGORIES.find((cat) => cat.value === subcategory)?.label || subcategory
    : '-';

  const getFieldLabel = (key: string) => {
    const translationKey = getShoppingGroceriesFieldTranslationKey(key);
    return t.has(translationKey as Parameters<typeof t>[0]) ? t(translationKey as Parameters<typeof t>[0]) : key;
  };

  const getOptionLabel = (value: string) => {
    const translationKey = getShoppingGroceriesOptionTranslationKey(value);
