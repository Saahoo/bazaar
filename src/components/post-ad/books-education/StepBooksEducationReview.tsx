...
import { BooksEducationSubcategory, BOOKS_EDUCATION_SUBCATEGORY_LABEL_KEYS, getBooksEducationFieldTranslationKey, getBooksEducationOptionTranslationKey } from '@/lib/constants/books-education-wizard';
import { BooksEducationMediaData } from './StepBooksEducationMedia';
import { UploadedPhoto } from '@/components/post-ad/ImageUploader';

interface StepBooksEducationReviewProps {
  locale: Locale;
  title: string;
  description: string;
...
export const StepBooksEducationReview: React.FC<StepBooksEducationReviewProps> = ({
  locale,
  title,
  description,
  subcategory,
  ...
  const subcategoryLabel = subcategory
    ? t(BOOKS_EDUCATION_SUBCATEGORY_LABEL_KEYS[subcategory] as Parameters<typeof t>[0])
    : '-';

  const getFieldLabel = (key: string) => {
    const translationKey = getBooksEducationFieldTranslationKey(key);
    return t.has(translationKey as Parameters<typeof t>[0]) ? t(translationKey as Parameters<typeof t>[0]) : key;
  };

  const getOptionLabel = (value: string) => {
    const translationKey = getBooksEducationOptionTranslationKey(value);
  ...
  // Build specs rows from the specs object, excluding common fields that are shown separately
