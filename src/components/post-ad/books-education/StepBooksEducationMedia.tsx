...
export interface BooksEducationMediaData {
  photos: UploadedPhoto[];
  videos?: string[]; // Optional video URLs
}

interface StepBooksEducationMediaProps {
  locale: Locale;
  data: BooksEducationMediaData;
  onChange: (data: Partial<BooksEducationMediaData>) => void;
}

export const StepBooksEducationMedia: React.FC<StepBooksEducationMediaProps> = ({ locale, data, onChange }) => {
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <ImageUploader
          ...
          maxPhotos={20}
          folder="listings/books-education"
        ...
        <div className={`border-2 border-dashed border-slate-300 rounded-xl p-8 text-center transition ${
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <button
                type="button"
                className="px-4 py-2.5 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition"
                onClick={() => {
      ...
      <div className="pt-6 border-t border-slate-200">
        <div className={`bg-indigo-50 border border-indigo-200 rounded-xl p-4 ${rtl ? 'text-right' : 'text-left'}`}>
          <h5 className="font-medium text-indigo-800 mb-2">{t('mediaTipsTitle')}</h5>
          <ul className={`space-y-1 text-sm text-indigo-700 ${rtl ? 'list-disc pr-5 pl-0' : 'list-disc pl-5'}`}>
            <li>{t('mediaTip1')}</li>
            <li>{t('mediaTip2')}</li>
            <li>{t('mediaTip3')}</li>
            <li>{t('mediaTip4')}</li>
