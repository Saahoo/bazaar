...
export interface BusinessIndustryMediaData {
  photos: UploadedPhoto[];
  videos?: string[]; // Optional video URLs
}

interface StepBusinessIndustryMediaProps {
  locale: Locale;
  data: BusinessIndustryMediaData;
  onChange: (data: Partial<BusinessIndustryMediaData>) => void;
}

export const StepBusinessIndustryMedia: React.FC<StepBusinessIndustryMediaProps> = ({ locale, data, onChange }) => {
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <ImageUploader
          ...
          maxPhotos={20}
          folder="listings/business-industry"
      ...
      <div className="pt-6 border-t border-slate-200">
        <div className={`bg-amber-50 border border-amber-200 rounded-xl p-4 ${rtl ? 'text-right' : 'text-left'}`}>
          <h5 className="font-medium text-amber-800 mb-2">{t('mediaTipsTitle')}</h5>
          <ul className={`space-y-1 text-sm text-amber-700 ${rtl ? 'list-disc pr-5 pl-0' : 'list-disc pl-5'}`}>
            <li>{t('mediaTip1')}</li>
            <li>{t('mediaTip2')}</li>
            <li>{t('mediaTip3')}</li>
            <li>{t('mediaTip4')}</li>
